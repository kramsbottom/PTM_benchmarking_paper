...
def unique(output, peptide_based,site_based,mass_based,non_collapse):
    ...
    df = pd.DataFrame({"Peptide_mod": peptide[1:], "Peptide": peptide_unmod[1:], "Protein": protein[1:], "Spectrum": spectrum[1:],"USI": USI[1:], "Source": source[1:], "Score": score[1:], "PTM": PTMs[1:],
    ...
    df=df.sort_values(['Peptide','Mass shift','Score','PTM_score'],ascending=[True,True,True,True])
    df['Pep-shift'] = df['Peptide'] +";"+df['Mass shift']
	...
	#Collapse by peptide
    df2 = pd.DataFrame({"Peptide_mod": peptide[1:], "Peptide": peptide_unmod[1:], "Protein": protein[1:], "Spectrum": spectrum[1:],"USI": USI[1:], "Source": source[1:], "Score": score[1:], "PTM": PTMs[1:],
         "PTM_score": PTM_score[1:],"PTM_positions": PTM_positions[1:],"PTM_Protein_Positions": PTM_protein_positions[1:], "PTM_info": PTM_info[1:], "Mass shift": mass_shift[1:], "FDR": FDR[1:], "Qvalue": q[1:]})
    df2=df2.sort_values(['Peptide','Score','PTM_score'],ascending=[True,True,True])
    ...
    df3 = pd.DataFrame({"Peptide_mod": peptide_split[1:], "Peptide":peptide_unmod_split[1:], "Protein": protein_split[1:],"Spectrum":spectrum_split[1:],"USI":USI_split[1:], "Source":source_split[1:], "Score": score_split[1:], "PTM": PTMs_split[1:],
    ...
    df3=df3.sort_values(['Protein','PTM_Protein_Position','Score','PTM_score'],ascending=[True,True,True,True])
    df3['Pro-pos'] = df3['Protein'] + "-" + df3['PTM_Protein_Position']
	...
	#No collapse
    df4 = pd.DataFrame({"Peptide_mod": peptide[1:], "Peptide": peptide_unmod[1:], "Protein": protein[1:], "Spectrum": spectrum[1:],"USI": USI[1:], "Source": source[1:], "Score": score[1:], "PTM": PTMs[1:],"PTM_score": PTM_score[1:],
    ...
    df4 = df4.sort_values(['Peptide', 'Score', 'PTM_score'], ascending=[True, True, True])
